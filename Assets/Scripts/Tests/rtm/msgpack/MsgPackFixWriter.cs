...
namespace GameDevWare.Serialization.MessagePack {
    ...
    public class MsgPackFixWriter : IJsonWriter {
        ...
        public MsgPackFixWriter(Stream stream, SerializationContext context, byte[] buffer = null) {
            if (stream == null) {
            ...
            if (buffer != null && buffer.Length < 32) {
                throw new ArgumentOutOfRangeException("buffer", "Buffer should be at least 32 bytes long.");
            }
        ...
        public void Write(string value) {
            ...
            try {
                bytes = this.context.Encoding.GetBytes(value);
            } catch (Exception ex) {}
            if (bytes == null) {
                this.WriteNull();
                return;
        ...
        public void WriteObjectBegin(int numberOfMembers) {
            if (numberOfMembers < 0) {
                throw new ArgumentOutOfRangeException("numberOfMembers");
            }
        ...
        public void WriteJson(char[] jsonString, int index, int charCount) {
            ...
            string json = null;
            try {
	            json = new String(jsonString, index, charCount);
            } catch(Exception ex){}
            this.Write(json);
        }
